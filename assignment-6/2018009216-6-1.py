...
def render():
    ...
    '''
    Rz1 = np.array([[np.cos(zang), -np.sin(zang), 0],
                   [np.sin(zang), np.cos(zang), 0],
                   [0,0,1]])
    Rz2 = np.array([[np.cos(yang), -np.sin(yang), 0],
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight, gAang, gBang, gRang
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018009216-6-1', None,None)
    if not window:
        glfw.terminate()
        return

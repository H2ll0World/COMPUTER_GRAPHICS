...
def display():
    ...
    # TODO: 여기서 카우가 움직이는 애니메이션을 구현하는 것
    # print
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, temp_cam2wld
    if isDrag: 
        print( "in drag mode %d\n"% isDrag);
        pp=pickInfo;
        if  isDrag==V_DRAG: # 수직드레그 구현해야 함
            # vertical dragging
            # TODO:
            ...
            if cursorOnCowBoundingBox:
                ray=screenCoordToRay(window, x, y);
                #pp=pickInfo; # 
                p=Plane(np.array((0,1,0)), pp.cowPickPosition)  # (0,1,0)은 normal vector
                c=ray.intersectsPlane(p);
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, '2018009216-PA2', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)

    '''
    TODO : onMousedrag, display 2군데이 있음
    '''
